...
├── launch-k8s-vms.sh         # Script tạo 3 control-plane + 2 worker
├── launch-nexus-vm.sh        # Script tạo Nexus node
└── README.md
```
...
### 1. Tạo các VM K8s (3 control-plane + 2 worker)

```bash
CLOUD_IMAGE="file:///path/to/Rocky-9.qcow2" bash vm-bootstrap/launch-k8s-vms.sh
```

### 2. Tạo Nexus VM

```bash
CLOUD_IMAGE="file:///path/to/Rocky-9.qcow2" bash vm-bootstrap/launch-nexus-vm.sh
```

> `CLOUD_IMAGE` là bắt buộc — script sẽ báo lỗi và thoát nếu không truyền vào.

### 3. Cài Docker + Nexus lên nexus node
...
```bash
# Xóa toàn bộ K8s VMs
multipass delete k8s-cp1 k8s-cp2 k8s-cp3 k8s-worker1 k8s-worker2 && multipass purge

# Xóa Nexus VM
multipass delete nexus && multipass purge

# Xóa tất cả
multipass delete k8s-cp1 k8s-cp2 k8s-cp3 k8s-worker1 k8s-worker2 nexus && multipass purge
```
